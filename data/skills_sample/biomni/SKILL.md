...
Use `resources/setup_environment.py` for interactive setup assistance.
...
See `resources/llm_providers.md` for detailed LLM configuration instructions.
...
See `resources/use_cases.md` for comprehensive task examples across all biomedical domains.
...
### References
Detailed documentation available in the `resources/` directory:
...
### Scripts
Helper scripts in the `resources/` directory:
...
# Or check .env.example file in working directory
```
...
- Documentation: Check `resources/` files for detailed guidance
